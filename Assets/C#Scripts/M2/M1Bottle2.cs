...
public class M1Bottle2 : MonoBehaviour

{
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.E) && checker)
        {
            transform.position = new Vector3(_player.transform.position.x, _player.transform.position.y, -15);
        ...
        else if (Input.GetKeyUp(KeyCode.E) || !checker)
        {
            if (checker)
            {
                Destroy(line);
